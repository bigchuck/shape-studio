...
class ShapeStudyRegistry:
    ...
    def dispatch(self, shape_names: List[str], points: List[Tuple[float, float]],
                 ...
                 generator) -> Tuple[List[Tuple[float, float]], Dict[str, Any]]:
        """
        ...
        for shape_name in shape_names:
            ...
            # Look up shape-specific config block
            shape_config = self._get_shape_config(name_lower)
            ...
            handler = self._handlers[name_lower]
            result = handler(
    ...
    def _get_shape_config(self, shape_name: str) -> Dict[str, Any]:
        ...
        Returns:
            Dictionary of shape-specific parameters with defaults applied
    ...
    def _pre_phase_u(self, points, centroid, bounds, shape_config,
        ...
        Args:
            ...
            bounds: Bounding box (x1, y1, x2, y2)
            shape_config: Dict from config.procedural.shapes.u
        ...
        # The reference line direction is along the segment.
        # Distance from centroid to the segment line gives us the
        # "available depth" for the indent.
        # ----------------------------------------------------------------
        ...
        for attempt in range(max_retries):
            try:
                ...
                metadata = {
                    ...
                    # These are indexed relative to the modified polygon
                    'significant_segments': [
    ...
    def _apply_u_indent(self, points, segment_idx, p1, p6,
        ...
        Args:
            points: Current polygon points
            segment_idx: Index of the segment being indented
            p1, p6: Original segment endpoints
            seg_length: Length of original segment
            seg_ux, seg_uy: Unit vector along segment direction
            perp_x, perp_y: Inward perpendicular unit vector
            centroid_distance: Distance from segment to centroid along perpendicular
            centroid: Polygon centroid (cx, cy)
            foot_width_min: Minimum foot width as fraction of segment (each side)
            foot_width_max: Maximum foot width as fraction of segment (each side)
            indent_min: Minimum indent depth as fraction of centroid distance
            indent_max: Maximum indent depth as fraction of centroid distance
            cross_centroid_prob: Probability that indent may cross centroid line
            depth_variation: Maximum variation between P3 and P4 depths (fraction)
            bounds: Bounding box for validation
            generator: ProceduralGenerators instance
        ...
        # foot_A is from P1 (t=0) to P2 (t=t_left)
        # foot_B is from P5 (t=t_right) to P6 (t=1)
        ...
        # Base depth as fraction of centroid distance
        ...
        # Maximum depth: up to centroid line, or beyond if crossing allowed
        if can_cross:
            max_depth = centroid_distance * 1.3  # Allow 30% beyond centroid
        else:
            max_depth = centroid_distance * 0.9  # Stay short of centroid
        ...
        min_depth = centroid_distance * indent_min * 0.5  # Minimum meaningful indent
        ...
        # Replace segment [segment_idx] -> [segment_idx+1] with 4 new points between
        ...
        if next_idx == 0:
            ...
            new_points = (
                ...
                points[next_idx:]             # P6 onward (P6 is already in points)
            )
    ...
    def _apply_foot_adjustment(self, points, segment_idx, perp_x, perp_y,
        ...
        So:
            ...
            P3 = points[segment_idx + 2]
            P4 = points[segment_idx + 3]
            P5 = points[segment_idx + 4]
            P6 = points[segment_idx + 5]
        ...
        Args:
            points: Polygon points (post-indent, with P1-P6 structure)
            segment_idx: Index of P1 in points
            perp_x, perp_y: Inward perpendicular unit vector
            centroid_distance: Distance from original segment to centroid
            foot_adjust_max: Maximum displacement as fraction of indent depth
            foot_extend_prob: Probability of extending vs reducing
            foot_min_remaining: Minimum remaining leg length in pixels
            bounds: Bounding box
            generator: ProceduralGenerators instance
        ...
        # Current indent depth (average of P3 and P4 depths for reference)
        p2 = points[idx_p2]
        p3 = points[idx_p3]
        p5 = points[idx_p4]  # P4 in the structure, but we want P5 for foot B
        ...
        if not extending:
            # Inner leg length for the chosen foot
            if foot_side == 'A':
                current_leg = inner_leg_a_length
            else:
                current_leg = inner_leg_b_length
            
            # Can't reduce more than would leave foot_min_remaining
            max_reduce = current_leg - foot_min_remaining
            if max_reduce <= 0:
                # Can't reduce at all - skip adjustment
                return points, {
                    'foot_side': foot_side,
                    'direction': 'skipped',
                    'displacement': 0,
        ...
        # Extend = move opposite to inward perpendicular (outward)
        # Reduce = move along inward perpendicular (inward / toward centroid)
        if extending:
